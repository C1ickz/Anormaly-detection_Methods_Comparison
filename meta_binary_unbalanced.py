'''
    Function: meta-classifier (voting) for binary classification on unbalanced dataset
...
def metaClassifier(trainset, trainlabels, testset):
    ...
    nn_clfer = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(5, 2), random_state=1).fit(train_data,
                                                                                                      train_labels)

    # pred_dt = dt_clfer.predict(testset)
    pred_nn = nn_clfer.predict(testset)
